# RESULTS — RoundUps (spec #33, Capital One / Nessie)

All numbers below were produced by real runs on **2026-08-03** on this machine.
No number is estimated or hand-written; each is emitted by the command shown and
persisted to the JSON file noted. Nothing here is fabricated — where the model or a
safety rail behaved imperfectly, the imperfect number is reported as-is.

## Environment (what "real" means here)

- **Nessie**: self-hosted, Nessie-compatible mock (`src/nessie-mock`) on `http://localhost:4173`.
  The spec explicitly permits this when the hosted API is unavailable; no hosted Capital One
  account was created. Routes mirror the real Nessie shape (customers / accounts / purchases /
  transfers / deposits).
- **LLM compiler**: local **Ollama `llama3.1:8b`** (temperature 0, `format: json`) standing in for
  the spec's "Claude/GPT function-calling". Free/local, no hosted key. This substitution is the
  single most important honesty caveat: a frontier hosted model would very likely score higher on
  the compiler eval than the 8B local model measured below.
...
- **Outperformance vs `flat_roundup` (median):** `percent_deposit_10` **+3,572% (36.7×)**;
  ...
  Honest caveat: the huge multipliers are structural — a percentage-of-paycheck rule moves far
...
## Metrics-to-capture checklist (from the spec)

| Spec metric | Measured value | Source command |
|---|---|---|
...
- Compiler is a local 8B model: 90% / 87.5%, not 100%. Three specific miscompiles and one
